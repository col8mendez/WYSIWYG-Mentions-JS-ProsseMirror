document.addEventListener("DOMContentLoaded", function () {
  ...
  function muevePunteoFinalDiv() {
    var ultimoParrafo = editor.lastElementChild;
  ...
  editor.addEventListener("click", function (evento) {
    ...
    for (var i = 0; i < evento.target.childNodes.length; i++) {
      if (
        evento.target.childNodes[i].localName == "img" &&
        evento.target.childNodes[i].alt == "Fin de la pagina"
      ) {
        var siguienteParrafo = evento.target.nextSibling;
        console.log(siguienteParrafo);
        var seleccion = window.getSelection();
        var rango = document.createRange();
        if (siguienteParrafo != null) {
          //si esto es nulo es porque no existe siguiente parrafo
    ...
    for (var i = 0; i < editorLimpiar.childNodes.length; i++) {
      if (editorLimpiar.childNodes[i].localName == "p") {
        ...
        for (var j = 0; j < nodoDiv.childNodes.length; j++) {
          if (
            nodoDiv.childNodes[j].localName == "img" &&
            nodoDiv.childNodes[j].alt == "Fin de la pagina"
          ) {
            for (var k = 0; k < nodoDiv.childNodes.length; k++) {
              if (
                nodoDiv.childNodes[k].localName != "br" &&
                nodoDiv.childNodes[k].alt != "Fin de la pagina"
              )
