...
var _SuggestState = class {
  ...
  constructor(suggesters) {
     ...
     */
    __privateAdd(this, _suggesters, void 0);
    /**
     ...
     */
    this.findNextTextSelection = (selection) => {
     ...
     */
    this.addIgnored = ({ from, name, specific = false }) => {
     ...
     */
    this.findMatchAtPosition = ($pos, name) => {
...
function suggest(...suggesters) {
  ...
  return new import_prosemirror_state3.Plugin({
    ...
    view: (view) => {
      ...
      return {
        update: (view2) => {
          return pluginState.changeHandler(view2.state.tr, false);
        }
      };
    },
    state: {
