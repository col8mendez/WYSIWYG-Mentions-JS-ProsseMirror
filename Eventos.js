...
document.addEventListener("DOMContentLoaded", () => {
    ...
    editor.addEventListener("click", (evento) =>{
        let eventParent = ObtieneEstiloTmFuenteTFuentePadre(evento);
        evento.target.style.length > 0 ?  obtenerEstilos(evento, evento.target.style, eventParent.style) : asignacionfuentes(); 
    },true)
    
    function ObtieneEstiloTmFuenteTFuentePadre(evento)
    {
        let etiqueta = evento.target.parentNode;
        let etiquetaResultado;
        while (etiqueta.localName !== "p") {
            if(etiqueta.style[0] !== undefined){
                if(etiqueta.style[0] != evento.target.style[0])
                    {
